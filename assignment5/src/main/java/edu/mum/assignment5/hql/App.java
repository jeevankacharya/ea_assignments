package edu.mum.assignment5.hql;
...
import edu.mum.assignment5.hql.model.Airline;
import edu.mum.assignment5.hql.model.Airplane;
import edu.mum.assignment5.hql.model.Airport;
import edu.mum.assignment5.hql.model.Flight;
...
//            List<Flight> flights = session.createQuery("from  Flight f join f.origin o where o.country = :country").list();
            Query query = session.createQuery("select f from  Flight f join f.origin o join f.airplane a  where o.country = :country and  a.capacity > :capacity");
        ...
        try {
            ...
            // TODO update HQL
            Query query = session.createQuery("Select distinct a from Airline a join a.flights f join f.airplane ap where ap.model = :model");
        ...
        try {
            ...
            // TODO update HQL
            Query query = session.createQuery("select f from Flight f where f.departureDate = :date AND f.departureTime < :time");
            query.setDate("date", new SimpleDateFormat("yyyy-MM-dd").parse("2009-08-07"));
            query.setTime("time", new SimpleDateFormat("HH:mm:ss").parse("12:00:00"));
            @SuppressWarnings("unchecked")
            List<Flight> flights = query.list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
